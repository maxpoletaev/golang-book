...
100». Может показаться странным то, что `x[4]` это пятый элемент массива, а не
четвертый но, как и строки, массивы нумеруются с нуля. Доступ к элементам
...
использовать имя типа в качестве функии.
...
закончится (позиция `high` НЕ включается!). Например: `arr[0:5]` вернет
`[1,2,3,4,5]`, `arr[1:4]` вернет `[2,3,4]`.

Для удобства, мы также может опустить `low`, `high` или и то и другое. `arr[0:]`
...
`x[1] = 10` она станет равна `1`. Во-вторых, карта не являются
...
Отображения часто используются для хранения общей информации. Давайте изменим
