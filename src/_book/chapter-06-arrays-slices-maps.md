...
В главе 3 мы изучили базовые типы Go. В этой главе мы рассмотрим еще три встроеннх типа: массивы, срезы и карты.
...
`x` это пример массива, состоящего из пяти элементов типа `int`. Запустим следующую програму:
...
Эта программы вычисляет среднюю оценку за экзамен. Если вы выполните её, то увидете `86.6`. Давайте рассмотрим её внимательнее:
...
Это был пример преобразования типов. В целом, для преобразования типа можно использовать имя типа в качестве функии.
...
Единственное отличие обявления среза от обьявления массива — отсутствие указания длины в квадратных скобках. В нашем случае `x` будет иметь длину 0.

Срез создается встроенной функцией `make`:

    x := make([]float64, 5)

Этот код создаст срез, который связан с массивом типа `float64` и длиной `5`. Среы всегда связаны с каким-нибудь массивом. они не можут стать больше чем массив, а вот меньше — пожалуйста. Функция `make` принимает и третий параметр:
...
`low` это позиция, с которой будет начинаться срез, а `high` это позиция, где он закончится (позиция `high` НЕ включается!). Например: `arr[0:5]` вернет `[1,2,3,4,5]`, `arr[1:4]` вернет `[2,3,4]`.
...
Карта представляется в связке с ключевым словом `map`, следующием за ним типом ключа в скобках и типом значения после скобок. Читается это следующим образом: «`x `это карта `string`-ов для `int`-ов».
...
Проблема нашей программы в том, что карта должна быть инициализирована передд тем, как будет использована. Надо написать так:

    x := make(map[string]int)
    x["key"] = 10
    fmt.Println(x["key"])

Если выполнить эту программу, то вы должны увидеть `10`. Выражение `x["key"] = 10` похоже на те, что использовались при работе с массивами, но ключ тут не число, а строка (потому что в карте указан тип ключа `string`). Мы также можем создать карты с ключем типа `int`:

    x := make(map[int]int)
    x[1] = 10
    fmt.Println(x[1])

Это выглядит очень похоже на мвссив, но существует несколько различий. Во-первых длина отображения (которую мы можем найти так: `len(x)`) может измениться, когда мы добавим новый элемент в него. В самом начале, при создании длина `0`, после `x[1] = 10` она станет равна `1`. Во-вторых, карта не являются последовательностью. В нашем примере у нас есть элемент `x[1]`, в случае массива должен быть и первый элемент `x[0]`, но отображениях это не так,
...
В данном примере, `elements` это карта, которое представляет 10 первых химических элементов, индексируемых символоами. Это очень частый способ использования карт — в качестве словаря, или таблицы. Предположим, мы пытаемся обратимся к не существующему элементу:
...
Обьявления отображения можно записывать скращенно, также как массивы:
...
Отображения часто используются для хранения общей информации. Давайте изменим нашу программу так, чтобы вместо имени элемента хранить какую-ниюудь дополнительную информацию о нем. Например, его агрекатное состояние:
