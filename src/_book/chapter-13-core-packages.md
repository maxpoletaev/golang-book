...
`Sum32()`, который вернёт `unit32`. Обычным применением `crc32` является сравнение
...
Одним из криптографических хэш-алгоритмов является HSA-1. Вот как можно его
использовать:
...
Когда мьютекс (`m`) заблокирован, любые попытки повторно блокировать его 
закончатся неудачей, до тех пор пока мьютекс не будет разблокирован. Следует 
проявлять большую осторожность при использовании мьютексов или примитивов 
синхронизации из пакета `sync/atomic`.

Традиционное многопоточное программирование является достаточно сложным: 
сделать ошибку просто, а обнаружит её трудно, поскольку она может зависеть от 
