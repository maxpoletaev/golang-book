...
Использование пакетов имеют смысл, только когда они востребованы в отдельной
...
файл `math.go` скомпилируется в объектный файл `~/Go/pkg/os_arch/golang-
book/chapter11/math.a` (при этом `os` — может быть `Windows`, a `arch`,
например, — amd64)
...
увидеть документацию в интернет страницы, запустив в терминале команду:
