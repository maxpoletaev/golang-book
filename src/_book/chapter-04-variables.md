...
Во-первых, когда мы видим символ `=`, мы по привычки читаем его как «х равен строке Hello World». Нет ничего неверного в том чтобы читать программу таким образом, но лучше читать это так: «х принимает значение строки Hello World» или «x присваивается строка Hello World». Это различие важно, потому что (как понятно по их названию) переменные могут менять свои значения во время выполнения программы. Попробуйте выполнить следующие:
...
Компилятор говорит вам, что переменная `x` внутри функции `f` не существует. Она существует только внутри функции `main`. Места, где может использоваться переменная `x` называется областью видимости переменной. Согласно спецификации, «В Go область видимости ограничена блоками». В основном, это значит, что переменные существуют в только внутри текущих фигурных скобок `{` `}` (в блоке), включая все вложенные скобки (блоки). Область видимости поначалу может запутать вас, но когда вы увидите больше примеров всё, станет ясно.
...
Тут мы используем другую функцию из пакета `fmt`, чтобы считать пользовательский ввод (`Scanf`). `&input` будет объяснен в следующий главах, и все что нам нужно знать сейчас — что `Scanf` заполняет переменную `input` введенным нами числом.
...
* Используя пример программы выше, напишите программа переводящей температуру из градусов Фаренгейта в градусы Цельсия. (`C = (F - 32) * 5/9`)
