...
В предыдущей главе мы использовали строковый тип данных, чтобы хранить `Hello
World`. Типы данных определяют множество принимаемых значений, описывают какие
операции могут быть применены к ним и определяют как данные будут храниться.
...
Предположим, у вас есть собака по имени Шарик. Тут «Шарик» это «Собака», этот
тип описывает какой-то набор свойств присущий всем собакам. Наши рассуждения тут
примерно следующие: у собак 4 лапы, Шарик — собака, значит у Шарика 4 лапы. Типы
данных в языках программирования работают похожим образом: все строки имеют
определенную длину, x — строка, значит x имеет определенную длину.
...
c.» в этом смысле множества схожи с типами данных в языках программирования в
том смысле, что все значения одног типа имеют общие свойства.

Go это язык программирования со статической типизацией. Это означает, что
переменные всегда меют определенный тип и этот тип нельзя изменить. Статическая
...
вещественной части. В отличии от десятичного предстваления чисел, которое
...
цифры, мы предствляем большое число используя новую цифру 2 (а затем 3, 4, 5, …)
...
выйти за лимит и большинство компьютеров просто вернуться к самому началу (что
...
сколько бит испрльзует каждый тип. `uint` означает «unsigned integer»
...
ноль). В дополнение к этому существуют два типа — псевдонима: `byte`, (тоже
самое, что `uint8`) и `rune` (тоже самое, что `int32`). Байты — очень
...
их размер зависит от архитектуры испрользуемого компьютера.
...
Числа с плавающей точкой это числа, которые содежат
вещественную часть (вещественные числа). (1.234, 123.4, 0.00001234, 12340000) Их
представление в компьютере довольно сложно и не особо необходимо для их
использования. Так что, мы просто должны помнить:

*   Числа с плавующей точкой неточны. Бывают случаи, когда число вобще нельзя
...
*   Как и целые числа, числа с плавающей точкой имеют определенный размер (32 бита
    или 64 бита). Использовение большего размера увеличивает точность (сколько цифр
    мы можем использовать для вычисления)

*   В дополнение к числам существуют несколько других значений, такие как: «not a
    number» (не число) (`NaN`, для вещей наподобии `0/0`) а также положительную и
    отрицательную бесконечность. (`+∞` и `−∞`)
...
дополнительных типа для представления комлексных чисел (чисел с мнимой частью):
...
«chapter3» с файлом main.go внутри, содержащий с следующим содержимым:
...
Заметим, что эта программа очень схожа программе, которую мы написали в главе 2.
Она содержит ту же строку с указанием пакета, ту же строку с импортом, то же
определение функции и использует ту же фунцию `Println`. В этот раз, вместо
печати строки `Hello World` мы печатаем строку `1 + 1 = ` с последующим
результатом выражения `1 + 1`. Это выражение состоит из трех частей: числовго
литерала `1` (который является типом `int`), оператора `+` (который предстваляет
...
Китайский, предствляются несколькими байтами)
...
использовать особые управлющие последовательности символов. Например, `\n` будет
заменен символом новой строки и `\t` будет заменен символом табуляции.

Распространенных операции на строках включают в себя нахождение длины строки:
`len("Hello World")`, доступ к отдельному символу в строке `"Hello World"[1]`, и
контетенации двух строк вместе: `"Hello " + "World"`. Давайте модифицируем
...
*   Строки “индексируются” начиная с 0, не с 1. [1] даст вам второй элемент, а не
    первый. Также заметьте, чтовы видете `101` вместо `e`, когда выполняете
...
*   Контатенация использует тот же символ, что и сложение. Компилятор Go выясняет
    что должно происходить полагаясь на типы аргументов. Если во обе стороны от `+`
    находятся строки, компилятор предположит, что вы имели в виду конкатенацию, а не
    сложение. (Ведь сложение для строк бесмысленно)

## Логические типы

Булевский тип (названный так в честь Джорджа Буля) это специальный однобитный
целочисленный тим, используемый для представления истинны и лжи. С этим типом
...
*   Мы знаем, что (в десятичной системе) самое большое число из 1 цифры это 9 и
    ...
    звух цифр это 11 (3), самое большое число из трех цифр это 111 (7) и самое
    большое число из 4 цифр это 1111 (15). Вопрос: какое самое больше число из 8
