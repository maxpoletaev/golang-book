...
Типы данных в языках программирования работают похожим образом: у все строк есть 
длина, `x` — строка, значит у `x` есть длина.
...
том смысле, что все значения одного типа имеют общие свойства.
...
вещественной части. В отличии от десятичного представления чисел, которое
...
*   В дополнение к числам существуют несколько других значений, такие как: «not a
    number» (не число) (`NaN`, для вещей наподобие `0/0`) а также положительную и
